...
- **Doctor**: read-only, dataset-wide diagnostics immediately after Action Insights. Its native TypeScript engine runs 12 checks over metadata, timing, actions, dimension-level continuity, video containers, statistics, episode consistency, training readiness, anomalies, and portability; it needs no Python runtime and can send affected episode IDs into the existing flagged-episode workflow.
