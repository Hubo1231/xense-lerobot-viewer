...
A hand-rolled dictionary + React context. **No i18n library, no `/[locale]/` route segment** — the URL shape (`/_local/<encoded>/episode_N`) is load-bearing and stays as it is.

- **Keys are flat and dotted** (`src/i18n/messages/en.ts`). `en` is the source of truth, `type MessageKey = keyof typeof en`, and `zh.ts` is typed `Record<MessageKey, string>` so tsc fails on a missing or stray key. Flat beats nested here: no recursive `Paths<T>` type, and a key greps as one string.
- **Plurals**: `_one` / `_other` pairs resolved by `pluralKey()`. Chinese has no plural and always takes `_other`; the `zh` `_one` entries are deliberately identical to their `_other`. Use `tp()` rather than reintroducing the `{n === 1 ? "" : "s"}` ternaries this replaced.
- **`tRich` / `tpRich`** (`src/i18n/rich.tsx`) interpolate **React nodes**, so a sentence that wraps one substitution in its own markup (a mono path, an amber count, a coloured tag word) stays a single translatable string. Do not split those into `…Before` / `…After` key pairs — that freezes English word order, which is exactly what differs in Chinese.
- **Preference lives in the `xense-locale` cookie**, read server-side in the root layout via `getServerLocale()`. That is what makes the first paint already correct and `<html lang>` right without a client round trip. The switcher writes the cookie **and** sets context state, so the UI flips with no `router.refresh()`. Reading the cookie makes the layout dynamic — every page here was already `force-dynamic`, so nothing is lost.
- **Font**: `Inter` is loaded as `--font-inter` and `globals.css` appends the CJK fallback chain on `body`. Inter ships no CJK glyphs; without the chain every Chinese string falls back per-glyph, usually to a serif.
- **Terminology**:
  - The brand's Chinese name is **千觉机器人**. It lives in `brand.part1` / `brand.part2` because the wordmark renders as two differently-coloured halves (千觉 / 机器人). The `Xense` that appears in a **dataset path** (`Xense/pack_bottles`) is a source directory, not the brand — never translate that.
  - **Everything about an episode stays English** — `{n} episodes`, `{n} ep`, `Episodes`, `Total Episodes`, `Per episode`. No Chinese measure word: the term is what the user also reads in the URL (`episode_N`), the parquet columns (`episode_index`) and `info.json` (`total_episodes`), so a translated count would not line up with any of them. A unit test rejects `条` after a number and the count noun `条数`; `条` inside unrelated words (`过滤条件`, `每行一条`, `空条目`) is fine.
  - The stat-row labels (`common.frames` / `tasks`) are only ever `<dt>`s above a figure, so they are count nouns: 帧数 / 任务数. `common.episodes` is `Episodes`, per the rule above.
  - **A unit that follows a number is written in English** — `{n} frames`, `{n} steps`, `{n} rows`, `{n} bytes`, `{n} days`, `{n} h`, `（s）`, `{r}×` — never 帧 / 步 / 行 / 字节 / 天 / 小时 / 秒 / 倍. A regression test rejects `{placeholder}` followed by a Chinese unit. Bare labels are nouns, not units, and stay Chinese (帧数, 首帧, 成功帧).
- **Strings thrown outside React**: the fetch clients (`doctorClient`, `syncClient`, `datasetTrashClient`, `versionUtils`, `subtasksClient`, `fetch-data`) throw `Error`s whose message a panel renders verbatim, so they are UI — but a plain module cannot call `useT()`. They use `tStandalone()` from `src/i18n/standalone.ts`, which reads the same `xense-locale` cookie the provider writes. Don't add a second locale source.
- **Three red lines — do not translate:**
  1. **Doctor's server-generated messages** (`src/lib/doctor/**`, ~146 `pass/warn/fail` call sites). `extractDoctorEpisodeIdsFromMessage` (`src/types/doctor.types.ts`) parses episode ids back out of the **English** message text, and `doctor-panel.tsx` branches on literal check names. Translating them silently breaks "flag affected episodes". Doing this properly means refactoring the builder to emit message codes + params first.
  2. **Vocabularies that get written to disk** — `COMMON_SKILLS` (`subtask-panel.tsx`), the preset tag lists (`src/lib/dataset-tags.ts`). Translating them changes stored dataset content. A display-only mapping is the only safe route.
  3. **Anything that came from the data** — dataset paths, task strings, parquet column names, robot types, URDF joint names.
- **Number/unit formatting is untouched.** `formatBytes` / `formatCompact` / `formatHours` / `formatDelta` stay pinned to `en-US`: the grouping is identical in `zh-CN`, and `GB` / `h` / `min` / `k` / `M` read fine in a Chinese technical UI. Chinese-idiomatic 万/亿 and `Intl.DateTimeFormat` dates are a separate, later step — don't thread a locale through those pure, unit-tested helpers piecemeal.
- **Fixed renderings** (asked for by name, keep them): 过滤 for "Filtering" (never 筛选), 动作审查 for "Action Insights".
- **Never branch on a translated string.** `verdict.label` in `action-insights-panel.tsx` used to drive the tip via `label === "Smooth"`; it now carries a separate `kind` discriminator. Same class of bug as the Doctor message parsing below — if a comparison reads user-facing text, it breaks the moment that text is translated.
- **Coverage**: every user-facing panel is translated — homepage (both levels, dashboard/tape, source panel, trash, tags editor), the viewer's tab bar and all nine tabs, both error pages, and the server-rendered integrity page (that one is a server component, so it looks the dictionary up via `getServerLocale()` + `MESSAGES` rather than React context).
- **What is still English, on purpose:**
  - **API-route error strings** (~15, across `src/app/api/**`) and the messages `scripts/*.py` print. Most are request-validation text that only fires on a broken client; the user-reachable ones (`"Dataset not found"`, `"Doctor timed out after 5 minutes."`, `export_subtasks.py`'s `die()` lines) surface in the tags / trash / sync / subtask dialogs. Translating them means giving every route a locale (and the Python scripts a `--locale`), which is the same plumbing the Doctor report needs — do both together or neither.
  - **Doctor's report body** — the ~146 `result.pass/warn/fail()` messages in `src/lib/doctor/**`, the 13 check names, and the "Copy details" clipboard export that quotes them. Three separate things read that text as data: `extractDoctorEpisodeIdsFromMessage` regexes episode ids out of it, `doctor-panel.tsx` branches on `check.name === "Dimension-Level Jump Detection"` / `"TCP Speed Limit Detection"`, and 24 assertions in `src/lib/doctor/__tests__/checks.test.ts` match English substrings (`"Episode 0"`, `/Episode \d+: .*action jump/`). Translating it also means threading a locale through the server route and into its result cache key. Doable, but it is a structured-message refactor (message codes + params), not a string swap — do that first if it is ever wanted.
  - **Schema tokens rendered as UI**: the v3.1 style names (`task_aug` / `subtask` / `plan` / `memory` / `interjection` / `speech` / `vqa`) in the annotation rail, timeline lanes and style pills. They are the values written to `language_persistent` / `language_events`. The quick-add dropdown shows the token plus a Chinese gloss (`subtask 子任务`) rather than replacing it.
  - **Annotation example placeholders** — `ann.ph.*` free-text examples keep their English sample content (that is the format that lands in the dataset); the Chinese entries just prefix them with `例：`.
- To extend or revise: add keys to both dictionaries, swap literals for `t()`, and let `src/i18n/__tests__/messages.test.ts` tell you what drifted — it asserts the set of byte-identical entries _exactly_, so both a new untranslated string and a stale exemption fail.
