...
Path resolution for all of these goes through `src/lib/local-dataset-paths.ts` (`resolveDatasetRoot` / `resolveInsideDataset` / `statDatasetFile`) — the traversal check lives there, not in each route.
...
- **Episode shortcut** — `locateEpisodeRows` walks `meta/episodes/chunk-*/file-*.parquet` (all chunks, not just chunk-000) for the row whose `episode_index` matches, and returns `data/chunk_index` + `data/file_index` + `dataset_from_index`/`dataset_to_index`. The tab opens on the current episode's own rows; the header button jumps back there. Returns null for v2.x (no such tree).
